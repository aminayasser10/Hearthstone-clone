...
public class herospells {
	...
	public static  void display(Hero cur , Hero opphero , Card c ){
		...
		x.getChildren().add(y);
		x.getChildren().add(z);
		BackgroundImage bg = new BackgroundImage(new Image("file:herospell.png"),
		...
		window.show();
			...
			player2heropic.setOnMouseClicked(e3 -> {
				HeroTargetSpell ss = (HeroTargetSpell) c;
				try {
					cur.castSpell(ss,
							opphero);sound2();
				} catch (NotEnoughManaException e1) {
